...
class MuFacNetBinarized(nn.Module):
    """
        Mutation Factorization Network (MuFacNet) with binarization
    """
...
class MutationFactorizationModel(torch_model.TorchModel):
    ...
    def compute_loss(self, data):
        ...
        # Convert to class probabilities, where the positive class is the first class.
        ...
        loss = self.loss_func(outputs, labels.to(torch.long).to(self.device))
        return loss
